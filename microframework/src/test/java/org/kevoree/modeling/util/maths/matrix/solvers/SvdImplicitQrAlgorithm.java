...
import org.kevoree.modeling.util.maths.matrix.solvers.decomposition.EigenvalueSmall;
...
//    SimpleMatrix B;
     ...
     * @return
     */
    public boolean processValues(double values[]) {
...
//            printMatrix();
    ...
    public boolean isOffZero(int i) {
        ...
        return Math.abs(off[i]) <= bottom* CommonOps.EPS;
    }
...
//        return Math.abs(diag[i]) <= maxValue* UtilEjml.EPS;
        ...
        return Math.abs(diag[i]) <= bottom* CommonOps.EPS;
    }
    ...
    /**
     * Tells it to processValues the submatrix at the next split.  Should be called after the
...
//            System.out.println();
    ...
    protected void eigenBB_2x2( int x1 ) {
        ...
        double sgn = signum(eigenSmall.value1.real);
